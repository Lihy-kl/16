...
namespace Lib_Card.ADT8940A1.Module.Extraction
{
    ...
    public class Extraction_Check : Extraction
    {
        public override int FluidExtraction(int iCylinderVersion, int iSyringeType, int iPulse, int iIsAssitant)
        {
            ...
            int iZRes = CardObject.OA1Axis.Absolute_Z(iSyringeType, Configure.Parameter.Other_Z_UpPulse, 0);
            if (0 != iZRes)
                return iZRes;

            try
            {
                iZRes = CardObject.OA1Axis.Absolute_Z(iSyringeType, Configure.Parameter.Other_Z_DownPulse, 0);
                if (0 != iZRes)
                    return iZRes;
            }
            catch (Exception ex)
