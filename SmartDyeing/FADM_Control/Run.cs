        ...
        private void table()
        {
            try
            {
                lock (this)
                {
                    ...
                    if (_b_select == false)
                    {
                        string s_sql = "select * from run_table order by MyID desc;";
                        dt_data = FADM_Object.Communal._fadmSqlserver.GetData(s_sql);
                    }

                    if (dt_data.Rows.Count > 0 && (this._i_max != dt_data.Rows.Count
                             ...
                             || this._b_reset))
                    {
                        ...
                        {
                            //计算总记录数
                            this._i_max = dt_data.Rows.Count;
                        ...
                        //克隆结构框架
                        DataTable dt_temp = dt_data.Clone();
                        ...
                        {
                            i_endpos = this._i_max;
                        }
                        else
                        {
                            i_endpos = this._i_pagesize * this._i_pagenow;
                        }

                        //计算开始行
                        if (this._i_pagelast != this._i_pagenow)
                        {
                            it_startpos = this._i_rownow;
                            this._i_rowlast = this._i_rownow;
                        }
                        else
                        {
                            it_startpos = this._i_rowlast;
                        }

                        //从数据源复制记录行
                        for (int i = it_startpos; i < i_endpos; i++)
                        {
                            dt_temp.ImportRow(dt_data.Rows[i]);
                            this._i_rownow++;
                        }
                        ...
                        {
                            //设置单元格格式
                            string[] sa_name = { "序号", "日期", "时间", "系统", "机械手", "染色机" };
                            int[] ia_width = { 100, 150, 150, 496, 496, 496 };
                        ...
                        {
                            //设置单元格格式
                            string[] sa_name = { "ID", "Date", "Time", "System", "Manipulator", "DyeingMachine" };
                            int[] ia__width = { 100, 150, 150, 496, 496, 496 };
                            for (int i = 0; i < sa_name.Length; i++)
                            {
                                this.dgv_Run.Columns[i].HeaderCell.Value = sa_name[i];
                                this.dgv_Run.Columns[i].Width = ia__width[i];
                            }
                        }
