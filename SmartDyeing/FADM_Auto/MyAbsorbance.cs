...
namespace SmartDyeing.FADM_Auto
{
    internal class MyAbsorbance
    {
        ...
        public static void Generate(int i_type,int i_cupNum)
        {
            ...
            foreach (DataRow dr in dataTable.Rows)
            {
                ...
                {
                    s_sql = "Insert into Abs_details(CupNum,Finish,Cooperate,GUID,StepNum,TechnologyName,Code,StirringRate,Dosage,StartingWavelength,EndWavelength,WavelengthInterval) Values(" + i_cupNum+",0,0,'"+ s_guid + "'," + dr["StepNum"].ToString()+",'"+ dr["TechnologyName"].ToString() + "','" + dr["Code"].ToString() + "',"+ dr["StirringRate"].ToString()+ ","+ dr["Dosage"].ToString() + "," + dr["StartingWavelength"].ToString() + "," + dr["EndWavelength"].ToString() + "," + dr["WavelengthInterval"].ToString() + ")";
                    FADM_Object.Communal._fadmSqlserver.ReviseData(s_sql);
                }
        ...
        private static readonly Dictionary<int, double> CIE_std_D65 = new Dictionary<int, double>
    {
        ...
        private static double CalculateKS(double reflectance)
        {
            return Math.Round(Math.Pow(1 - reflectance, 2) / (2 * reflectance), 6);
        }
        ...
        public static double SWL(double[] sampleAbsorbance, double[] standardAbsorbance)
        {
            ...
            {
                sampleReflectance = Math.Round(Math.Pow(10, -sampleAbsorbance[maxAbsorbanceIndexStandard]), 6);
                standardReflectance = Math.Round(Math.Pow(10, -standardAbsorbance[maxAbsorbanceIndexStandard]), 6);
            }
            else
            {
                sampleReflectance = Math.Round(Math.Pow(10, -sampleAbsorbance[maxAbsorbanceIndexSample]), 6);
                standardReflectance = Math.Round(Math.Pow(10, -standardAbsorbance[maxAbsorbanceIndexStandard]), 6);
            }
        ...
        public static double SUM(double[] sampleAbsorbance, double[] standardAbsorbance, int startWavelength, int endWavelength, int interval)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    // 将吸光度转化为反射率
                    double sampleReflectance = Math.Round(Math.Pow(10, -sampleAbsorbance[i]), 6);
                    double standardReflectance = Math.Round(Math.Pow(10, -standardAbsorbance[i]), 6);
        ...
        public static double WSUM(double[] sampleAbsorbance, double[] standardAbsorbance, int startWavelength, int endWavelength, int interval, int angle)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (currentWavelength >= 400 && currentWavelength <= 700)
                {
                    ...
                    double fx = CIE_std_D65[currentWavelength] * cieData[currentWavelength].X;
                    double fy = CIE_std_D65[currentWavelength] * cieData[currentWavelength].Y;
                    double fz = CIE_std_D65[currentWavelength] * cieData[currentWavelength].Z;

                    // 计算 K/S 值
                    double ksSample = CalculateKS(sampleReflectance) * (fx + fy + fz);
                    double ksStandard = CalculateKS(standardReflectance) * (fx + fy + fz);
